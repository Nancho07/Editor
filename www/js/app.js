// Hernan Guevara 12 feb-2016
//var DATAURL = "https://indicadores-imixhn.c9users.io/phpmovil/instancias/";


/***************** Editor ***************************/
window.addEventListener("load", function(){
        //first check if execCommand and contentEditable attribute is supported or not.
        if(document.contentEditable != undefined && document.execCommand != undefined){
           alert("HTML5 API de edición no es soportado");
        }else{
            document.execCommand('styleWithCSS', false, true);
        }
    },false);    
    //underlines the selected text    
    /*function underline(){ 
        document.execCommand("underline", false, null);
    }*/    
    /*function bold(){
        document.execCommand("bold", false, null);
    }*/
    function backcolor(value){
        document.execCommand("backColor", false, value);
    }
    function forecolor(value){
        document.execCommand("foreColor", false, value); 
    }
    /*function backcolorCat(value){
        document.execCommand("backColor", false, value);
    }
    function backcolorEst(value){
        document.execCommand("foreColor", false, value); 
    }*/
    function heading(value){
        document.execCommand("heading", false, value);
    } 
    function fontsize(value){
        var size1 = parseInt(value);      
        document.execCommand("fontSize", false, size1);
    } 
    //displays HTML of the output
    
    function guardarNota(){
        var editor =  document.getElementById("editor").innerHTML;
        socket.emit("guardarNota",editor);
    }

/****************FIN EDITOR ************************/ 
function setName(){
        if(conectado !== "No hay coneccion"){
            datos_usuario.uuid=device.uuid;
            socket.emit('identify', datos_usuario);
        }
    }
function select_prefix(value){
    datos_usuario.prefix=value;
}
function onAppReady(){  
    var underLineBtn = document.getElementById('underLineBtn');
    underLineBtn.addEventListener('click', function () {
        document.execCommand('underline', false, null);
    });
    var bold = document.getElementById('boldBtn');
    bold.addEventListener('click', function () {
        document.execCommand('bold', false, null);
    });
    var jr = document.getElementById('jrBtn');
    jr.addEventListener('click', function () {
        document.execCommand("JustifyRight", false, "");
    });    
    var jl = document.getElementById('jlBtn');
    jl.addEventListener('click', function () {
        document.execCommand("JustifyLeft", false, "");
    });    
    var jc = document.getElementById('jcBtn');
    jc.addEventListener('click', function () {
        document.execCommand("JustifyCenter", false, "");
    });  
    var ja = document.getElementById('jAll');
    ja.addEventListener('click', function () {
        document.execCommand("justifyFull", false, "");
    });  
       
    $(document).on('click', '#enviar',function(e){ 
        e.preventDefault();
        if(conectado != "No hay coneccion" && conectado!=undefined){
            var pass=MD5($('#pass').val()),
                user=$('#user').val();  
            var formData = "user="+user+"&pass="+pass+"&prefix=imix";
            datos_usuario.usuario=user;
            datos_usuario.clave=pass;
            datos_usuario.web="NO";
            setName();
            return false;
        }else{
            $("body").pagecontainer("change", "#geo_datos", {
                transition: 'flip',
                changeHash: false,
                reverse: true,
                showLoadMsg: true
            }); 
            return false;
        }
    });
    $(document).on('click', '#inicio',function(e){ 
        e.preventDefault();
        $("body").pagecontainer("change", "#datos1", {
            transition: 'flip',
            changeHash: false,
            reverse: true,
            showLoadMsg: true
        });
        return false;
    });
    $(document).on('pageinit', "#geo_datos", function () {
        pbar = jQMProgressBar('progressbar')
                .setOuterTheme('b')
                .setInnerTheme('e')
                .isMini(true)
                .setMax(100)
                .setStartFrom(0)
                .setInterval(10)
                .showCounter(true)
                .build();        
    });
    $(document).on('complete', '#progressbar', function () {  
        $('#msg').text("Completado!");
        setTimeout(
          function(){
            $('#msg').text("Esperando");
            pbar.setValue(0);
          }, 5000);
    });
    /*****************************************************************************************************************************/
    $(document).on('click', '.cargar_imagenes',function(){ 
        socket.emit("cargarImg",datos_usuario);
    });
    /****************************************************************************************************************************/
    $(document).on('click', '#cuadros',function(e){                
        $("body").pagecontainer("change", "#cuadricula", {
            transition: 'flip',
            changeHash: false,
            reverse: true,
            showLoadMsg: true
        });
    });

    $(document).on('click', '#galeria_fotos img',function(e){  
        var nan = $(this).attr("src");
        var name = $(this).attr("miImagen");
        var nombre =name.split(".");
        var rafa=nan.replace("thumbs","originales");
        var mary=rafa.replace("thumb_","");
        $("#bigFoto").attr("src",mary);

        $("body").pagecontainer("change", "#bigImg", {
            transition: 'flip',
            changeHash: false,
            reverse: true,
            showLoadMsg: true
        }); 
        $("#nombreImg").val(name);
    });
    //cuadrosImgEditor
    $(document).on('click', '#gridCuadros img',function(e){   
        var nan = $(this).attr("src");
        var name = $(this).attr("miImagen");
        var nombre = name.split(".");
        var rafa=nan.replace("thumbs","originales");
        var mary=rafa.replace("thumb_","");

        $("#bigFoto").attr("src",mary);
        $("body").pagecontainer("change", "#bigImg", {
            transition: 'flip',
            changeHash: false,
            reverse: true,
            showLoadMsg: true
        });
        $("#nombreImg").val(name);
    });
    $(document).on('click', '#cuadrosImgEditor img',function(){        
        var titulo_img = $("#tituloFoto").val();
        var url = $(this).attr("src");
        
        if(titulo_img=="Foto Contenido"){
            document.execCommand("InsertImage", false, url);
            // Get the current selection
            var sel = window.getSelection(); 
            if (sel.rangeCount > 0) {
                var range = sel.getRangeAt(0);
                var node = range.startContainer;
                if (node.hasChildNodes() && range.startOffset > 0) {
                    node = node.childNodes[range.startOffset - 1];
                }
                // Walk backwards through the DOM until we find an image
                while (node) {
                    if (node.nodeType == 1 && node.tagName.toLowerCase()  == "img") {
                        //alert("Found inserted image with src " + node.src);
                        node.style.width='50%';
                        node.style.margin='4px 4px 4px 4px';
                        node.style.cssFloat = "left";
                        node.setAttribute("class", "foto_contenido");
                        /*.style.cssFloat = "right";*/
                        break;
                    }
                    node = previousNode(node);
                }
            }
        }else if(titulo_img=="Foto Portada"){
           $("#fotoPortada").val(url); 
           $("#fotoPortadaImg").attr("src",url);
        }
    });
    /*$(document).on('click', '#btnConfigNuevo0',function(){   
        function onSuccess(position) {
            $("#longitud").val(position.coords.longitude);
            $("#latitud").val(position.coords.latitude);
        }
        function onError(error) {
            console.log('code: '    + error.code    + '\n' +
                  'message: ' + error.message + '\n');
        }
        var watchID = navigator.geolocation.watchPosition(onSuccess, onError, { timeout: 30000 });        
    });*/

    var request = indexedDB.open("geodatos", 1);
        var db = null;
        request.onupgradeneeded = function(){
            db = request.result;
            var store = db.createObjectStore("geodatos_store", {keyPath: "lugar_k", autoIncrement: true});
            store.createIndex("contacto", "contacto", {unique: false});
            store.createIndex("telefono", "telefono", {unique: false});
            store.createIndex("categoria", "categoria", {unique: false});
            store.createIndex("nombre", "nombre", {unique: false});
            store.createIndex("direccion", "direccion", {unique: false});
            store.createIndex("longitud", "longitud", {unique: false});
            store.createIndex("latitud", "latitud", {unique: false});
            store.createIndex("foto", "foto", {unique: false});

        }            
        request.onsuccess = function(){
            console.log("Coneccion establecida con la base de datos");
            db = request.result;
        }
        request.onerror = function(){ 
            console.log("Un error a ocurrido al conectar con la base de datos"); 
        }
    $(document).on('click', '#btnCargarDatos0',function(){ 
        socket.emit("cargarCategorias",datos_usuario);
        socket.emit("cargarEstados",datos_usuario);
        socket.emit("cargarNotas",datos_usuario);
    });

    $(document).on('click', '#data_cargada li',function(){
        $("#lugar_k").val($(this).attr("id"));
        $("#latitud").val($(this).attr("data-lat"));
        $("#longitud").val($(this).attr("data-long"));
        $("#contacto").val($(this).attr("data-cont"));
        $("#telefono").val($(this).attr("data-tel"));
        $("#local").val($(this).attr("data-nombre"));
        $("#direccion").val($(this).attr("data-direc"));
        $("#nombreImg").val($(this).attr("miImagen"))
        var catego = $(this).attr("data-catego");
        var el = document.getElementById(catego);
            el.selected = true; 
        $('select').selectmenu("refresh", true);
    });

    $(document).on('click', '#nuevo_registro',function(){
        $("#lugar_k").val(0);
        $("#latitud").val(vlatitud);
        $("#longitud").val(vlongitud);
        $("#contacto").val("");
        $("#telefono").val("");
        $("#local").val("");
        $("#direccion").val("");
        $("#nombreImg").val("");
    });
    
    $(document).on('click', '#gridCuadros input',function(){
        var id = $(this).attr("id");
    });
    $(document).on('click', '#guardar_local',function(){
         if($('#contacto').val()!=="") {
            var pass=MD5($('#pass').val()),
                user=$('#user').val(),
                direccion=$('#direccion').val(),
                catego=$('#catego').val(),
                telefono=$('#telefono').val(),
                local=$('#local').val(),
                contacto=$('#contacto').val(),
                lugar_k=$('#lugar_k').val(),
                latitud=$('#latitud').val(),
                longitud=$('#longitud').val();
            function onConfirm(buttonIndex) { 
                var write_transition = db.transaction("geodatos_store", "readwrite");
                var store = write_transition.objectStore("geodatos_store");
                if(buttonIndex==1){
                    var id=lugar_k;
                    var object1 = {contacto: contacto, telefono: telefono, categoria: catego, nombre: local,direccion:direccion,longitud:longitud,latitud:latitud,foto:""};
                    store.put(object1);
                }else if(buttonIndex==2){
                    var object1 = {contacto: contacto, telefono: telefono, categoria: catego, nombre: local,direccion:direccion,longitud:longitud,latitud:latitud,foto:"",lugar_k:lugar_k};
                    store.put(object1);
                }else if(buttonIndex==3){
                    var deleted_row = store.delete(lugr_k);
                    deleted_row.onsuccess = function(event){
                        navigator.notification.confirm(
                             '',
                             '', // funsion de accion
                            'Se elimino el registro',
                            ['Salir']
                        );
                    }
                }
            }   
            navigator.notification.confirm(
                'Selecciona el destino',
                 onConfirm, 
                'Guardar datos Locales',
                ['Crear','Actualizar','Borrar','Salir']
            );
         }
    });    
    
    /****************************  color Picker ********************************/
    var cv = document.getElementById('colorpick');
    var ccepc = document.getElementById('colorCatEstPick');
    var ctx = cv.getContext('2d');
    var ctxccepc = ccepc.getContext('2d');
    var img = new Image();
    // Meh .. Thx 2 Browser Security i need BASE64
    img.src = 'data:image/gif;base64,';

    img.onload = function() {
        ctx.drawImage(img, 0, 0, img.width, img.height);
        ctxccepc.drawImage(img, 0, 0, img.width, img.height);
    };

    // Dirty Colorpicker
    $('#colorpick').on('mousedown', function(eventDown) {
        var x = eventDown.offsetX;
        var y = eventDown.offsetY;

        if (eventDown.button === 0) {
            $('.card.menu > .header > img').addClass('excite');
            setTimeout(function() {
                $('.card.menu > .header > img').removeClass('excite');
            }, 800);

            var imgData = ctx.getImageData(x, y, 1, 1).data;
            localStorage.setItem('color', JSON.stringify(imgData));
            stylechange(imgData); 
        }
    });
    $('#colorCatEstPick').on('mousedown', function(eventDown) {
        var x = eventDown.offsetX;
        var y = eventDown.offsetY;

        if (eventDown.button === 0) {
            $('.card.menu > .header > img').addClass('excite');
            setTimeout(function() {
                $('.card.menu > .header > img').removeClass('excite');
            }, 800);

            var imgData = ctx.getImageData(x, y, 1, 1).data;
            localStorage.setItem('color', JSON.stringify(imgData));
            stylechangeCatEst(imgData); 
        }
    });
    function stylechange(arr) {
        var x = 'rgba(' + arr[0] + ',' + arr[1] + ',' + arr[2] + ',1)';
        var cb = document.getElementById('colorBox');
        cb.style.backgroundColor = x;
        if($("#tituloColor").val()=="Color de letra"){
            forecolor(x);
        }else{
            backcolor(x);
        }
    }
    function stylechangeCatEst(arr) {
        var x = 'rgba(' + arr[0] + ',' + arr[1] + ',' + arr[2] + ',1)';
        
        if(document.getElementById("catColorFondo").checked==true){        
            var cb = document.getElementById('boxColorCatEst');
            cb.style.backgroundColor = x;
            var cb0 = document.getElementById('nombreConfig');
            cb0.style.backgroundColor = x;
        }else if(document.getElementById("catColorTexto").checked==true){
            var cb = document.getElementById('boxColorCatEst');
            cb.style.color = x;
            var cb0 = document.getElementById('nombreConfig');
            cb0.style.color = x;
        }
        var valor=$("#idConfig").text();
        var nombre = valor.substring(0,3);
        
        if(nombre=="cat"){
            $('#boxColorCatEst').text="Categoría";
        }else if(nombre=="est"){
            $('#boxColorCatEst').text="Estado";
        }
    }
    // Dyncolor ftw
    if (localStorage.getItem('color') !== null) {
        var colorarray = JSON.parse(localStorage.getItem('color'));
        stylechange(colorarray);
    } else {
        var colorarray = [15, 157, 88]; // 15 157 88 = #0f9d58
        localStorage.setItem('color', JSON.stringify(colorarray));
        stylechange(colorarray);
    }
    
    $(document).on('click', '.colorSelect',function(){ 
        if($(this).attr("id")=="fonColorBtn"){
            $("#tituloColor").val("Color de letra");
        }else{
            $("#tituloColor").val("Color de fondo");
        }         
        uib_sb.toggle_sidebar($("#colorPickerBox"));    
         return false;
    });   
    
    $(document).on('click', '.fotoPortada',function(){ 
        if($(this).attr("id")=="fotoPortadaImage"){
            $("#tituloFoto").val("Foto Portada");
        }else if($(this).attr("id")=="fotoContenidoImage"){
            $("#tituloFoto").val("Foto Contenido");
        }         
        uib_sb.toggle_sidebar($("#imagesEditor"));    
         return false;
    });
    $(document).on('click', '#btnGuardarCategoria',function(){ 
        editorial.catego = $("#nuevaCategoria").val();
        editorial.prefix = datos_usuario.prefix;
        editorial.publicar_movil = 1;
        editorial.fecha_creacion = new Date();
        //editorial.principal = $("#catAct").val();
        datos_usuario.editorial = editorial;
        if(datos_usuario.editorial.catego !="" && datos_usuario.editorial.catego !=undefined){
            socket.emit("guardarCategoria",datos_usuario); 
        }       
    });
    $(document).on('click', '#btnGuardarEstado',function(){ 
        editorial.estado = $("#nuevoEstado").val();
        editorial.prefix = datos_usuario.prefix;
        editorial.publicar_movil = 1;
        editorial.fecha_creacion = new Date();
        datos_usuario.editorial = editorial;
        if(datos_usuario.editorial.estado !="" && datos_usuario.editorial.estado !=undefined){
            socket.emit("guardarEstado",datos_usuario); 
        }       
    });
    $(document).on('click', '#htmlBtn',function(){ 
        if(document.getElementById('edithtml').style.display=="none"){            
            document.getElementById('edithtml').style.display="";
            document.getElementById('edithtml').style.height="200px";
            var editor =  document.getElementById("editor").innerHTML;
            document.getElementById("edithtml").value = editor.trim();
            document.getElementById('editor').style.display="none";            
        }else{
            document.getElementById('editor').style.display="";
            var html = document.getElementById("edithtml").value;
            document.getElementById("editor").innerHTML = html.trim();
            document.getElementById('edithtml').style.display="none";
            document.getElementById('edithtml').style.height="200px"; 
            
        }
    });
    
     $(document).on('click', '#saveBtn',function(){ 
         var salvar = {};
        //if(document.getElementById('edithtml').style.display=="none"){
            salvar.fecha_creacion = new Date();
            salvar.prefix = datos_usuario.prefix;
            salvar.nota_k = document.getElementById("nota_k").value;
            salvar.socketId = datos_usuario.socketId;
            salvar.titulo_movil =  document.getElementById("tituloMovil").value;
            salvar.contenido =  document.getElementById("editor").innerHTML;
            salvar.catego =  document.getElementById("categoriaEditorial").value;
            salvar.estado =  document.getElementById("estadosEditorial").value;
            salvar.foto_portada =  document.getElementById("fotoPortada").value;
            socket.emit("guardarEditorial",salvar);
        //}
    });
    
    $(document).on('click', '#linkear',function() { 
        var person = prompt("ingrese una url", "");
        if (person != null && person !="") {
            var url = person;
            var selection = document.getSelection();
            document.execCommand('createLink', true, url);
            selection.anchorNode.parentElement.target = '_blank'; 
        }
    });
    
    $(document).on("click","#btnConfigEditorial",function(){
        $("body").pagecontainer("change", "#configuracion", {
                    transition: 'flip',
                    changeHash: false, 
                    reverse: true,
                    showLoadMsg: true
                });
    });
    $(document).on("click","#btnPru",function(){
        $("body").pagecontainer("change", "#configuracion", {
                    transition: 'flip',
                    changeHash: false,
                    reverse: true,
                    showLoadMsg: true
                });
    });
    
    $(document).on("click","#btnEditorialConfig",function(){
        $("body").pagecontainer("change", "#geo_datos", {
                    transition: 'flip',
                    changeHash: false,
                    reverse: true,
                    showLoadMsg: true
                });
    });
    $(document).on("click","#btnEditorialGaleria",function(){
        $("body").pagecontainer("change", "#galeria", {
                    transition: 'flip',
                    changeHash: false,
                    reverse: true,
                    showLoadMsg: true
                });
    });
    $(document).on("click","#btnBorrarConfig",function(){    
        var datos = {};
         if($("#idConfig").val()!=""){
            datos.idConfig = $("#idConfig").val();
            datos.nombreConfig = $("#nombreConfig").val();
            datos_usuario.borrarConfig = datos;
            var r = confirm("Seleccione su orden!");
            if (r == true) {
                socket.emit("borrarConfig",datos_usuario);
            } else {
                alert("Ha cancelado el comando borrar");
            }
            
       }            
    });
     $(document).on("click","#btnGuardarConfigCatEst",function(){    
        var datos = {};
         if($("#idConfig").val()!=""){
            datos.idConfig = $("#idConfig").val();
            datos.nombreConfig = $("#nombreConfig").val();
            datos.bgcolor = document.getElementById('nombreConfig').style.backgroundColor;
            datos.color = document.getElementById('nombreConfig').style.color;
            datos.principal=$("#catAct").val();
            datos.orden = $("#catOrder").val();
            
             datos_usuario.guardarConfig = datos;
           
            var r = confirm("Seleccione su orden!");
            if (r == true) {
                socket.emit("guardarConfig",datos_usuario);
            } else {
                alert("Ha cancelado el comando guardar");
            }
            
       }            
    });
    $(document).on("click","#btnConfigNuevo0",function(){ 
        document.getElementById("nota_k").value = 0;
        document.getElementById("tituloMovil").value = "";
        document.getElementById("editor").innerHTML = "";
        document.getElementById("categoriaEditorial").value = 1;
        document.getElementById("estadosEditorial").value=1;
        document.getElementById("fotoPortada").value="images/imix.png";
        $("#fotoPortadaImg").attr("src","images/presionar.png"); 
        
        function onSuccess(position) {
            $("#longitud").val(position.coords.longitude);
            $("#latitud").val(position.coords.latitude);
        }
        function onError(error) {
            console.log('code: '    + error.code    + '\n' +
                  'message: ' + error.message + '\n');
        }
        var watchID = navigator.geolocation.watchPosition(onSuccess, onError, { timeout: 30000 });  
    });
    $(document).on("click","#btnNotificacion",function(){ 
        var fecha = new Date();
        var salvar = {};
        salvar.fecha_creacion = fecha;
        salvar.prefix = datos_usuario.prefix;
        salvar.socketId = datos_usuario.socketId;
        salvar.titulo_movil =  document.getElementById("tituloMovil").value;
        salvar.contenido =  document.getElementById("editor").innerHTML;
        socket.emit("oneSignal",salvar);    
    });
    function selectCategorias(element){    
        $("#idConfig").val(element);
        $("#nombreConfig").val($("#"+element).text());
        $("#catAct").val($("#"+element).find(".principal").prop("checked"));
        $("#catOrder").val($("#"+element).find(".order").val());
        
        var cb0 = document.getElementById(element),
            bgColor = cb0.style.backgroundColor,
            bgText = cb0.style.color;
        $("#nombreConfig").css("background-color",bgColor);
        $("#nombreConfig").css("color",bgText);
    }
    function selectEstados(element){    
        $("#idConfig").val(element);
        $("#nombreConfig").val($("#"+element).text());
        
        var cb0 = document.getElementById(element),
            bgColor = cb0.style.backgroundColor,
            bgText = cb0.style.color;
        $("#nombreConfig").css("background-color",bgColor);
        $("#nombreConfig").css("color",bgText);
    }
    function selectNotas(element){    
        $("#idConfig").val(element);        
        $("#nota_k").val(element);
        $("#nombreConfig").val($("#"+element).text());
         var id = element.substring(3);
        $("#nota_k").val(parseInt(id));
        datos_usuario.nota_k=parseInt(id);
        socket.emit("cargarNota",datos_usuario);
        
        var cb0 = document.getElementById(element),
            bgColor = cb0.style.backgroundColor,
            bgText = cb0.style.color;
        $("#nombreConfig").css("background-color",bgColor);
        $("#nombreConfig").css("color",bgText);
    }
    /*******************************FIN COLOR BOX*******************************************/
    socket.on('warning', function (data) {
        if(data.acceso=="Acceso Permitido"){
            $(descripcion).empty();
            descripcion = '<li class="widget uib_w_88" data-uib="jquery_mobile/listitem" data-ver="0" data-role="list-divider"><span>'+data.success +'</span></li>'+
            '<li class="widget uib_w_88" data-uib="jquery_mobile/listitem" data-ver="0" data-role="list-divider"><span>'+ data.acceso+'</span></li>'+   
            '<li class="widget uib_w_88" data-uib="jquery_mobile/listitem" data-ver="0" data-role="list-divider"><span>'+ data.text+'</span></li>'+
            '<li class="widget uib_w_88" data-uib="jquery_mobile/listitem" data-ver="0" data-role="list-divider"><span>'+ data.body+'</span></li>';
            $(descripcion).appendTo($("#description"));
            if (data.success=="true"){              
                $("#bienvenido").empty();
                $("#bienvenido").append("Bienvenido "+data.name);
                $("body").pagecontainer("change", "#geo_datos", {
                    transition: 'flip',
                    changeHash: false,
                    reverse: true,
                    showLoadMsg: true
                });
                socket.emit("cargarCategorias",datos_usuario);
                socket.emit("cargarEstados",datos_usuario);
            }else{
                descripcion = '<li class="widget uib_w_88" data-uib="jquery_mobile/listitem" data-ver="0" data-role="list-divider"><span> Acceso:'+data.succees+'</span></li>';
                $(descripcion).appendTo($("#description"));
            }
        }
        return false;
    });
    socket.on("nuevaCategoria",function(datos){
        $("#nuevaCategoria").empty();
        if(datos.catego!="" && datos.catego!=undefined){ 
            $("#categoriaEditorial").append('<option value='+datos.id+'>'+datos.catego+'</option>');
            $('#categoriaEditorial').selectmenu('refresh', true);
        }
    });
    socket.on("nuevoEstado",function(datos){
        $("#nuevoEstado").empty();
        if(datos.estado!="" && datos.estado!=undefined){
            $("#estadosEditorial").append('<option value='+datos.id+'>'+datos.estado+'</option>');
            $('#estadosEditorial').selectmenu('refresh', true);
        }
    });
    socket.on("allCategorias",function(datos){
        $("#categoriaEditorial").empty();        
        $("#listaCategorias").empty();
        $.each(datos.rows,function(index, value){
            $("#categoriaEditorial").append('<option value='+value.catego_k+'>'+value.nombre+'</option>');            
            var cln = $("#listaCategoriasResp li:first").clone();
                $(cln).attr("id", "cat"+value.catego_k);
                $("#listaCategorias").append(cln);
                $("#cat"+value.catego_k).text(value.nombre);
                $("#cat"+value.catego_k).text(value.nombre);                
                $("#cat"+value.catego_k).css("color",value.catego_text_color);
                $("#cat"+value.catego_k).css("backgroundColor", value.catego_color);
                $("#cat"+value.catego_k).bind("click",function(){
                    selectCategorias($(this).attr("id"));
                });
                $("#cat"+value.catego_k).append('<input type="checkbox" class="principal" style="width:20px; height:20px;">');
                $("#cat"+value.catego_k).append('<input type="number" class="order"style="width:45px; height:20px;">');
                $("#cat"+value.catego_k).find(".principal").css("position","relative");
                $("#cat"+value.catego_k).find(".principal").css("float","right");
                
                $("#cat"+value.catego_k).find(".order").css("position","relative");
                $("#cat"+value.catego_k).find(".order").css("float","right");
                
                $("#cat"+value.catego_k).find(".order").val(value.orden)
            
                $("#cat"+value.catego_k).css("height","50px");
                if(value.principal==="true"){
                    $("#cat"+value.catego_k).find(".principal").prop("checked", true);
                }else{
                    $("#cat"+value.catego_k).find(".principal").prop("checked", false);
                }                
        });
        $('#categoriaEditorial').selectmenu('refresh', true);
    }); 
    socket.on("allEstados",function(datos){
        $("#estadosEditorial").empty();
        $("#listaEstados").empty();
        $.each(datos.rows,function(index, value){
            $("#estadosEditorial").append('<option value='+value.id+'>'+value.estado+'</option>');

            var cln = $("#listaEstadosResp li:first").clone();
            $(cln).attr("id", "est"+value.id);
            $("#listaEstados").append(cln);
            $("#est"+value.id).text(value.estado);
            $("#est"+value.id).bind("click",function(){
                selectEstados($(this).attr("id"));
            });            
        });
        $('#estadosEditorial').selectmenu('refresh', true);
    });
    socket.on("allNotas",function(datos){
        $("#listaNotas").empty();
        $.each(datos.rows,function(index, value){
            var cln = $("#listaNotasResp li:first").clone();
            $(cln).attr("id", "not"+value.nota_k);
            $("#listaNotas").append(cln);
            $("#not"+value.nota_k).text(value.titulo_movil);
            $("#not"+value.nota_k).bind("click",function(){
                selectNotas($(this).attr("id"));
            });            
        });
    });
    socket.on("notaGuardada",function(datos){
        if(datos.text=="Guardado"){
            alert(datos.text);
            document.getElementById("nota_k").value=datos.id;
        }else{
            alert(datos.text);
        }
    });    
    socket.on("notaCargada",function(datos){
        $.each(datos.rows,function(index, value){
            document.getElementById("tituloMovil").value = value.titulo_movil;
            document.getElementById("editor").innerHTML = value.contenido_movil;
            document.getElementById("categoriaEditorial").value = value.catego_k;
            document.getElementById("estadosEditorial").value=value.estado;
            document.getElementById("fotoPortada").value=value.foto_portada_movil;
            $("#fotoPortadaImg").attr("src",value.foto_portada_movil);
        });
    });
    
    socket.on("cargar_imagenes",function (data,url){
        var imagenes = "";
        var imagenesC = "";
        $("#galeria_fotos").empty();
        $("#gridCuadros").empty();
        $("#cuadrosImgEditor").empty();
        $.each(data,function(index, value){
            var nombre = value.nombre;
            nombre = nombre.replace("thumb_", "");
            imagenes=imagenes+"<li href='#bigImg'><div class='caja' align='center'><img src="+
                value.image+" miImagen="+nombre+"></br><span align = 'center' style='font-size:11px'>"+nombre+"</span></div></li></br>";
            imagenesC=imagenesC+"<div class='widget widget-container content-area vertical-col uib-card cpad-0 uib_w_49 section-dimension-49 caja' data-uib='layout/card' data-ver='0'><input type='checkbox' name='"+nombre+"' id='"+nombre+"' data-mini='true' style='top:4px'><img src="+value.image+" style='padding:3px 3px 3px 3px'  miImagen="+nombre+"></br><span style='font-size:9px'>"+nombre+"</span></div>";
        });
        $(imagenes).appendTo($("#galeria_fotos"));
        $(imagenesC).appendTo($("#gridCuadros"));
        $(imagenesC).appendTo($("#cuadrosImgEditor"));
        return false;
    });
}
document.addEventListener("app.Ready", onAppReady, false);
